...
if(isset($_GET['edit_account'])){
    $user_session_name=$_SESSION['username'];//Récupère le username depuis la session

    $select_query="Select * from `user_table` where username='$user_session_name'";
    $result_query=mysqli_query($con,$select_query);
    $row_fetch=mysqli_fetch_assoc($result_query);// Cherche les données de l'utilisateur en BDD

    $user_id=$row_fetch['user_id'];
    $username=$row_fetch['username'];
    $user_email=$row_fetch['user_email'];
    $user_address=$row_fetch['user_address'];
    $user_mobile=$row_fetch['user_mobile']; //Extrait chaque donnée dans une variable pour pré-remplir le formulaire
}
    ...
    if(isset($_POST['user_update'])){
        ...
        $user_image_tmp=$_FILES['user_image']['tmp_name']; // Récupère les nouvelles valeurs saisies dans le formulaire
        ...
        $result_query_update=mysqli_query($con,$update_data);//Met à jour toutes les colonnes en BDD
       
        if($result_query_update){
            echo "<script>alert('Data updated successfully')</script>"; //Si succès → déconnecte l'utilisateur
